      ...
      <b-navbar toggleable="lg" class="naviStyle" type="dark">
        ...
        <b-collapse id="nav-collapse" is-nav>
          <b-navbar-nav class="mx-auto">
            ...
            <b-nav-item v-if="!store.currentUser" to="/AboutUs">ABOUT US</b-nav-item>
            <b-nav-item v-if="!store.currentUser" to="/Contact">CONTACT</b-nav-item>
            <b-nav-item v-if="!store.currentUser" to="/Calendar">CALENDAR</b-nav-item>
            <b-nav-item v-if="!store.currentUser" to="/News">NEWS</b-nav-item>
          </b-navbar-nav>

          <ul class="nav">
            <b-dropdown v-if="store.currentUser" id="dropdown-right" variant="primary" size="lg" right text="Profile"
              class="m-2">
              <b-dropdown-item to="/edit">{{ store.userDisplayName }}
                <span v-if="store.userIsAdmin"> - admin</span>
                <span v-else> - user vulgaris</span>
              </b-dropdown-item>
              <b-dropdown-item to="/signup" @click.prevent="logout()">Log Out</b-dropdown-item>
            </b-dropdown>

            <b-button pill style="background-color:#384f7b" to="/login" v-if="!store.currentUser">Login</b-button>
            <pre> </pre>
