<template>
  <div class="mb-3">
    <div class="d-inline-block" style="padding-left: 35px">
      <b-card style="width: 200px" body-class="text-center" :title="naziv" :img-src="slika" :img-alt="naziv" img-top>
        ...
        <b-button v-if="userIsAdmin === false" v-on:click="submit(price)"
          style="background-color: #384f7b; border-color: #384f7b; bottom: 20px" block pill bottom>Choose plan</b-button>
        <!-- otkrij kako otvoriti samo jedan modal i editirati podatke -->

        <b-button v-if="userIsAdmin === true" v-on:click="editSub(price)"
          style="background-color: #384f7b; border-color: #384f7b; bottom: 20px" block pill bottom>Edit</b-button>
        <b-button v-if="userIsAdmin === true" v-on:click="delSub(price)" variant="danger" block pill
          bottom>Delete</b-button>
      </b-card>
    </div>
  </div>
</template>
